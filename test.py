...
class CaptionTester:
    def __init__(self, model_dir: str, device: Optional[str] = None):
        ...
        projector_path = self.model_dir / "projector.pt"
...
def test(config_path: str):
    ...
    if not os.path.exists(caption_config.clip_model_path):
    ...
    tester = CaptionTester(config.model_dir)

    if config.image_path:
        caption = tester.generate_caption(config.image_path, max_new_tokens=config.max_new_tokens)
